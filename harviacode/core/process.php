...
if (isset($_POST['generate']))
{
    ...
    if ($table_name <> '')
    {
        ...
        $modules = $get_setting->modules;
        
        
        //mkdir($target . $c_url, 0777, true);
        if (!file_exists($modules . $c_url . "/views/"))
        {
            mkdir($modules . $c_url . "/views/", 0777, true);
        }
        if (!file_exists($modules . $c_url . "/controllers/" ))
        {
            mkdir($modules . $c_url . "/controllers/", 0777, true);
        }
        if (!file_exists($modules . $c_url . "/models/"))
        {
            mkdir($modules . $c_url . "/models/", 0777, true);
        }
        if (!file_exists($modules . $c_url . "/config/"))
        {
            mkdir($modules . $c_url . "/config/", 0777, true);
        }
        if (!file_exists($modules . $c_url . "/helpers/"))
        {
            mkdir($modules . $c_url . "/helpers/", 0777, true);
        }
        if (!file_exists($modules . $c_url . "/libraries/"))
        {
            mkdir($modules . $c_url . "/libraries/", 0777, true);
        }
